...
public class StuAction {
	...
	void stuUpdate() {
		while (true) {
			...
			if (count == -1) {
				System.out.printf("※※ 찾고자 하는 %s학생이 없습니다. ※※ 다시 입력하세요.!! \n", name);
				System.out.println();
			} // if
	...
	void stuDelete() {
		while (true) {
			...
			System.out.printf("삭제 하고자 하는 학생 이름을 입력하세요.(1.이전페이지 이동)\n");
			String name = scan.next();
			if (name.equals("0")) { // 이전페이지이동
				System.out.println(">> 이전페이지를 이동합니다!!");
				System.out.println();
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (name.equals(s.getName())) {
					...
					case 1:
						...
						break;
					case 2:
						System.out.printf("%s 학생성적 삭제가 취소되었습니다.!\n", name);
						System.out.println();
						break;
	...
	void stuSearch() {
		while (true) {
			...
			System.out.printf("검색 하고자 하는 학생 이름을 입력하세요.(1.이전페이지 이동)\n");
			String name = scan.next();
			if (name.equals("0")) { // 이전페이지이동
				System.out.println(">> 이전페이지를 이동합니다!!");
				System.out.println();
